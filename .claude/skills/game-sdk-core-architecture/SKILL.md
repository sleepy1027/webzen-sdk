...
* `glz::read`/`glz::read_json` 기본 옵션(`error_on_unknown_keys = true`)은 모르는 키를 만나면 그 자리에서 파싱을 멈춘다 — 그 뒤에 나오는, 그 타입이 원래 알고 있는 필드조차 못 읽는다. 그래서 **`glz::read_json`을 직접 쓰지 않는다** — 대신 `core/request.hpp`의 `webzen::ReadJson(value, json)`을 쓴다 (`error_on_unknown_keys = false`가 이미 적용된 얇은 래퍼, 시그니처/리턴값은 `glz::read_json`과 동일해서 그냥 이름만 바꿔 끼우면 됨). 요청/응답/로컬 저장소 등 이 SDK가 스키마를 100% 통제하지 못하는 JSON을 읽는 곳은 전부 여기 해당한다 — 서버가 필드 하나 추가했다고, 혹은 예전 버전이 저장해둔 토큰에 새 필드가 없다고 파싱이 깨지면 안 된다. 이 프로젝트에서 한 번 실제로 이 순서 의존성 때문에 `RequestId` correlation이 조용히 깨지는 버그가 났었고 (`CommandRegistry::Dispatch`), 이후 `AuthService`의 서버 응답/로컬 저장소 파싱에서도 같은 위험이 있는 걸 확인해서 전부 `ReadJson`으로 통일했다.
